#set text(font: "TTLivretText-Rg", size: 11pt)
#set par(
  spacing: 1.65em,
  justify: true,
)
#set page(
  footer: context [
    #set align(right)
    #set text(weight: "bold", font: "TTLivretText-DmBd", size: 15pt)
  ...
  set text(font: "TTLivretText-DmBd", size: 18pt)
    ...
    .find(h => h.location().page() == here().page()) == none {
      ...
      if smh.len() > 0 {
        ...
        let m = measure(ht)
        place(top + left, dx: 2.1cm, dy: 1.5cm, float: false, ht)
      } else {
        ""
      }
    } else {
...
#show raw: set text(font: "Berkeley Mono", size: 11pt)
#set raw(theme: "agola.tmTheme")

#align(center + horizon)[
#align(center)[
  #text(size: 5.5em)[On Lisp]
]

#align(center)[
  #text(size: 2em)[Paul Graham]
]
...
#set page(
  margin: (left: 3.3cm, right: 2cm, top: 1.5cm, bottom: 3cm),
)
...
Even from a distance, the shapes of `bad-` and `good-reverse` suggest which is the better program. And despite being shorter, `good-reverse` is also more efficient: O(n) instead of O(n²).
...
Built upon it is `symb`, which is mostly used for building symbols. It takes one or more arguments and returns the symbol (creating one if necessary) whose printname is their concatenation. It can take as an argument any object which has a printable representation: symbols, strings, numbers, even lists.
...
The backquote character, #raw("`"), is so named because it resembles a regular quote, ', reversed. When backquote alone is affixed to an expression, it behaves just like quote:

#raw("`(a b c)") is equal to #raw("'(a b c)").
...
#raw("`(a b c)") is equal to #raw("(list 'a 'b 'c)").
...
#raw("`(a ,b c ,d)") is equal to #raw("(list 'a b 'c d)").
...
One comma counteracts the effect of one backquote, so commas must match backquotes. Say that a comma is surrounded by a particular operator if the operator is prepended to the comma, or prepended to an expression which contains it. In #raw("`(,a ,(b ',c)))"), for example, the last comma is surrounded by one comma and two backquotes. The general rule is: a comma surrounded by n commas must be surrounded by at least n+1 backquotes. An obvious corollary is that commas may not appear outside of a backquoted expression. Backquotes and commas can be nested, so long as they obey the rule above. Any of the following expressions would generate an error if typed into the toplevel:
...
2. The object to be spliced must be a list, unless it occurs last. The expression #raw("`(a ,@1)`") will evaluate to `(a . 1)`, but attempting to splice an atom into the middle of a list, as in #raw("`(a ,@1 b)"), will cause an error.
...
For an example of macro where this distinction matters, see the note on page 393.
...
The code in Figure 7.8 isn't exactly the way `do` would be written in a real implementation. To emphasize the computation done during expansion, `make-initforms` and `make-stepforms` have been broken out as separate functions. In the future, such code will usually be left within the `defmacro` expression.
...
Unfortunately, the `let` technique works only in a narrow range of cases: macros where 1. all the arguments at risk of capture are evaluated exactly once, and 2. none of the arguments need to be evaluated in the scope of bindings established by the macro skeleton.
...
The implicit block in `do` is not a problem, because `do` is advertised to behave this way. However, you should realize that if you write macros which expand into `do`s, they will capture the block name `nil`. In a macro like `for`, a `return` or `return-from nil` will return from the `for` expression, not the enclosing block.
...
A Bezier curve is defined in terms of four points—two endpoints and two control points. When we are working in two dimensions, these points define parametric equations for the x and y coordinates of points on the curve. If the two endpoints are (x₀, y₀) and (x₃, y₃) and the two control points are (x₁, y₁) and (x₂, y₂), then the equations defining points on the curve are:
x = (x₃ − 3x₂ + 3x₁ − x₀)u³ + (3x₂ − 6x₁ + 3x₀)u² + (3x₁ − 3x₀)u + x₀
y = (y₃ − 3y₂ + 3y₁ − y₀)u³ + (3y₂ − 6y₁ + 3y₀)u² + (3y₁ − 3y₀)u + y₀

If we evaluate these equations for n values of u between 0 and 1, we get n points on the curve. For example, if we want to draw the curve as 20 segments, then we would evaluate the equations for u = .05, .1,. . ., .95. There is no need to evaluate them for u of 0 or 1, because if u = 0 they will yield the first endpoint (x₀, y₀), and if u = 1 they will yield the second endpoint (x₃, y₃).

An obvious optimization is to make n fixed, calculate the powers of u beforehand, and store them in an (n−1) × 3 array. By defining the curve-generator as a macro, we can do even better. If n is known at expansion-time, the program could simply expand into n line-drawing commands. The precomputed powers of u, instead of being stored in an array, could be inserted as literal values right into the macro expansion.
...
With n = 20, `genbez` expands into 21 `setf`s. Since the powers of u appear directly in the code, we save the cost of looking them up at runtime, and the cost of computing them at startup. Like the powers of u, the array indices appear as constants in the expansion, so the bounds-checking for the `(setf aref)`s could also be done at compile-time.
...
The definition of `aand` shows that the expansion will vary depending on the number of arguments in the macro call. If there are no arguments, then `aand`, like the regular `and`, should simply return `t`. Otherwise the expansion is generated recursively, each step yielding one layer in a chain of nested `aif`s:
...
Here we show a similar but cleaner solution. The read-macro in Figure 17.5 defines an expression of the form `#{f₁ f₂ ... fₙ}` to read as the composition of f₁, f₂, ..., fₙ. Thus:
...
As in a `with-slots`, the variables now refer to the corresponding locations in the structure. There is one important difference, however: you must use `setf` rather than `setq` to set these pseudo-variables. The `with-slots` macro must invoke a code-walker (page 273) to transform `setq`s into `setf`s within its body. Here, writing a code-walker would be a lot of code for a small refinement.
...
Two further improvements appear because the new version calls `destruc` (Figure 18.1). The pattern can now contain `&rest` or `&body` keywords (`match` doesn't bother with those). And because `destruc` uses the generic sequence operators `elt` and `subseq`, the new `if-match` will work for any kind of sequence. If `abab` is defined with the new version, it can be used also on vectors and strings:
...
Also, expressions of the form (or q₁ ... qₙ) are only guaranteed to generate real bindings for variables which appear in all of the qᵢ. If a `with-answer` contained the query
...
Continuations can be understood as a generalization of closures. A closure is a function plus pointers to the lexical variables visible at the time it was created. A continuation is a function plus a pointer to the whole stack pending at the time it was created. When a continuation is evaluated, it returns a value using its own copy of the stack, ignoring the current one. If a continuation is created at T₁ and evaluated at T₂, it will be evaluated with the stack that was pending at T₁.
...
Many programs need to find a path from one node in a tree to one of its descendants. Suppose we have a function `kids` which returns a list of the children of a node, or `#f` if there are none. We want to write a function `(descent n1 n2)` which returns a list of nodes on some path from n1 to its descendant n2, if there is one. Figure 22.1 shows a deterministic version of this function.
...
What does this ATN do when given the input (spot runs)? The first node has one outgoing arc, a `cat`, or category arc, leading to node `s2`. It says, effectively, you can follow me if the current word is a noun, and if you do, you must store the current word (indicated by `*`) in the `subj` register. So we leave this node with `spot` stored in the `subj` register.
...
The predefined register operators work on the current, or topmost, set of registers: `getr` reads a register; `setr` sets one; and `pushr` pushes a value into one. Both `getr` and `pushr` use the primitive register manipulation macro `set-register`. Note that registers don't have to be declared. If `set-register` is sent a certain name, it will create a register with that name.
...
If
(hungry ?x) and (smells-of ?x turpentine)
Then (painter ?x)
...
```
If
(species ?x) and (big ?x) and (fierce ?x)
Then (rare ?x)
```
...
(<- (append (?x . ?xs) ?ys (?x . ?zs))
    (append ?xs ?ys ?zs))
```
caption: [Prolog syntax equivalence.]
...
The case of `append` points to a great difference between Prolog and other languages. A collection of Prolog rules does not have to yield a specific value. It can instead yield constraints, which, when combined with constraints generated by other parts of the program, yield a specific value. For example, if we define `member` thus:
